const validateIdent: any = {
    ...
    IdentityCode_checkProvince(card: string) { //取身份证前两位，校验省份    
        var province: any = card.substr(0, 2);
        if (validateIdent.aIdentityCode_City[province] == undefined) {
            return false;
        }
        return true;
    },
    IdentityCode_checkBirthday(card: any) { //检查生日是否正确，15位以'19'年份来进行补齐。  
        var len = card.length;
    ...
    IdentityCode_verifyBirthday(year: any, month: number, day: number, birthday: Date) {//校验日期 ，15位以'19'年份来进行补齐。
        var now = new Date();
    ...
    IdentityCode_checkParity(card: any) { //校验位的检测  
    ...
    IdentityCode_changeFivteenToEighteen(card: any) {  //15位转18位身份证号   
        if (card.length == '15') {
            var arrInt = new Array(7, 9, 10, 5, 8, 4, 2, 1, 6, 3, 7, 9, 10, 5, 8, 4, 2);
            var arrCh = new Array('1', '0', 'X', '9', '8', '7', '6', '5', '4', '3', '2');
            var cardTemp = 0, i;
